...
import { 
  Plus, Edit2, Loader2, Trophy, Trash2, Image as ImageIcon, Calendar, Crop,
...
const PrestasiRow = memo(({ prestasi, onEdit, onDelete }: { prestasi: any, onEdit: (p: any) => void, onDelete: (id: string) => void }) => (
  <tr>
    <td className="ps-3 py-2">
      <div className="d-flex align-items-center">
        <div className="ui-thumb-container flex-shrink-0">
          {prestasi.foto_url ? (
            <img src={`${prestasi.foto_url}?t=${prestasi.updated_at || Date.now()}`} alt="thumb" className="w-100 h-100 object-cover block" />
          ) : (
    ...
    <td className="py-2 text-muted text-[10px] d-none d-md-table-cell">
      <div className="d-flex align-items-center">
        ...
        {prestasi.tahun || '-'} ({prestasi.tingkat || '-'})
      </div>
    </td>
    <td className="py-2 text-end pe-3">
      <div className="d-flex justify-content-end gap-1">
        <button onClick={() => onEdit(prestasi)} className="btn btn-sm p-1 text-warning border-0 shadow-none" title="Edit Prestasi">
          <span className="bg-light p-1 rounded-3 d-inline-flex"><Edit2 size={11}/></span>
        </button>
        <button onClick={() => onDelete(prestasi.id)} className="btn btn-sm p-1 text-danger border-0 shadow-none" title="Hapus Prestasi">
...
PrestasiRow.displayName = 'PrestasiRow';

export default function ManajemenPrestasi() {
  ...
  const fetchData = useCallback(async (page = 1) => {
    ...
    setLoading(true);
    try {
      const res = await api.admin.prestasi.getAll({ page });
      if (res?.data) {
  ...
  const handleApplyCrop = async () => {
    if (tempImage && croppedAreaPixels) {
      try {
        ...
        const file = new File([croppedBlob], "prestasi_foto.jpg", { type: "image/jpeg" });
  ...
  const handleCloseForm = useCallback(() => {
    ...
    setFormData({ judul: '', tahun: '', tingkat: '', kategori: 'Sekolah', foto: null });
  }, [photoPreview]);

  const handleEditClick = useCallback(async (p: any) => {
    setIsEdit(true); setCurrentId(p.id);
    setFormData({ 
      judul: p.judul || '', 
      tahun: p.tahun || '', 
      tingkat: p.tingkat || '', 
      kategori: p.kategori || 'Sekolah', 
      foto: null 
    });
    setPhotoPreview(p.foto_url || null);
    setShowForm(true);
    if (p.foto_url) {
      try { const base64 = await getBase64FromUrl(p.foto_url); setOriginalImage(base64); } catch (e) { console.error(e); }
    }
  }, []);
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        const res = await api.admin.prestasi.delete(id);
        if (res.status === 200 || res.data?.success) {
          Toast.fire({ icon: 'success', title: res.data?.message || 'Prestasi dihapus' });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEdit && currentId) {
        ...
        res = await api.admin.prestasi.update(currentId, payload);
      } else {
        res = await api.admin.prestasi.create(payload);
      }
  ...
  return (
    <div className="container-fluid py-3 px-2 px-md-3">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <div className="d-flex align-items-center">
          <Trophy size={16} className="text-warning me-2" />
          <h6 className="mb-0 fw-bold text-dark text-uppercase text-[12px] tracking-wider">Prestasi Sekolah</h6>
        </div>
        <button onClick={() => setShowForm(true)} className="btn btn-warning btn-sm px-2 px-md-3 shadow-sm rounded-3 py-1.5 text-[10px]">
          <Plus size={13} className="me-1"/> <span>Tambah Prestasi</span>
        </button>
      </div>
          ...
          <table className="table table-hover align-middle mb-0">
            <thead className="bg-light">
              <tr className="text-[9px]">
                <th className="ps-3 border-0 py-2.5 fw-bold text-muted text-uppercase">Nama Prestasi</th>
                <th className="border-0 py-2.5 fw-bold text-muted text-uppercase d-none d-md-table-cell">Tahun & Tingkat</th>
            ...
            <tbody className="border-top-0">
              {loading ? (
                ...
                <tr>
                  <td colSpan={3} className="text-center py-5 text-muted text-[10px]">Tidak ada data prestasi.</td>
                </tr>
              ) : data.map((p) => (
                <PrestasiRow key={p.id} prestasi={p} onEdit={handleEditClick} onDelete={handleDelete} />
              ))}
            </tbody>
          </table>
        </div>
            ...
            <div className="modal-content border-0 shadow-lg rounded-3 overflow-hidden">
              <div className="modal-header border-0 pb-0 px-3 pt-3">
                <h6 className="modal-title fw-bold text-dark text-[12px]">
                  {tempImage ? "Potong Foto" : (isEdit ? "Edit Prestasi" : "Input Prestasi")}
                </h6>
              ...
              <div className="modal-body p-3 pt-2">
                {tempImage ? (
                  ...
                  <>
                    <div className="mb-2">
                      <label className="form-label text-dark mb-1 fw-semibold text-[10px]" htmlFor={judulId}>Judul Prestasi</label>
                    ...
                    <div className="row g-2">
                      <div className="col-6 mb-2">
                        <label className="form-label text-dark mb-1 fw-semibold text-[10px]" htmlFor={tahunId}>Tahun</label>
                        <input id={tahunId} type="number" className="form-control bg-light border-0 shadow-none py-1.5 px-3 text-[10px] rounded-3" value={formData.tahun} onChange={(e) => setFormData({...formData, tahun: e.target.value})} />
                        {errors.tahun && <div className="text-danger mt-1 text-[8px]">{errors.tahun[0]}</div>}
                      </div>
                      <div className="col-6 mb-2">
                        <label className="form-label text-dark mb-1 fw-semibold text-[10px]" htmlFor={tingkatId}>Tingkat</label>
                        <input id={tingkatId} type="text" className="form-control bg-light border-0 shadow-none py-1.5 px-3 text-[10px] rounded-3" placeholder="Nasional, Prov, dll" value={formData.tingkat} onChange={(e) => setFormData({...formData, tingkat: e.target.value})} />
                      </div>
                    </div>
                    <div className="mb-2">
                    ...
                    <div className="mb-2">
                      <label className="form-label text-dark mb-1 fw-semibold text-[10px]" htmlFor={fotoId}>Foto Utama</label>
                      ...
                      {photoPreview && (
                        <div className="ui-preview-foto" onClick={() => originalImage && setTempImage(originalImage)}>
                          ...
                          <button type="button" className="btn-close ui-btn-delete-preview shadow-none" onClick={(e) => { e.stopPropagation(); setFormData({...formData, foto: null}); setPhotoPreview(null); setOriginalImage(null); }} aria-label="Remove photo"></button>
                        </div>
                      )}
                    </div>
                  </>
              ...
              {!tempImage && (
                <div className="modal-footer border-0 p-3 pt-0">
                  <button onClick={handleSave} className="btn btn-warning btn-sm w-100 fw-bold shadow-sm py-2 text-[11px] rounded-3" disabled={isSubmitting}>
                    {isSubmitting ? <Loader2 size={12} className="animate-spin" /> : (isEdit ? "Update Prestasi" : "Simpan Prestasi")}
                  </button>
                </div>
              )}
            </div>
