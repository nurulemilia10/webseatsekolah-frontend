...
import { Plus, Edit2, Loader2, Calendar, Trash2, ChevronLeft } from "lucide-react";
import api from "@/lib/api";
import Swal from "sweetalert2";
import { useAuth } from "@/hooks/useAuth";

const SemesterRow = memo(
  ({
    item,
    onEdit,
    onDelete,
    ...
    <tr>
      ...
      <td className="py-2 text-muted text-[10px] d-none d-md-table-cell">
        <div className="d-flex align-items-center">
          <span
            className={`badge ${item.is_active ? "bg-success" : "bg-secondary"} bg-opacity-10 text-${item.is_active ? "success" : "secondary"} border-0 text-[9px]`}
          >
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        const errorMsg =
          e.response?.data?.errors?.relasi?.[0] ||
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.status === 200 || res.status === 201 || res.data?.success) {
        Toast.fire({
          icon: "success",
          title: res.data?.message || "Berhasil disimpan",
        });
        handleCloseForm();
      ...
      if (e.response?.status === 422) {
        ...
        Toast.fire({
          icon: "error",
          title: e.response?.data?.message || "Terjadi kesalahan server",
        });
      }
    } finally {
    ...
    <div className="container-fluid py-3 px-2 px-md-3">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <div className="d-flex align-items-center">
          ...
          <h6 className="mb-0 fw-bold text-dark text-uppercase text-[12px] tracking-wider">
            Manajemen Semester
          </h6>
        </div>
        <button
          onClick={() => setShowForm(true)}
          className="btn btn-warning btn-sm px-2 px-md-3 shadow-sm rounded-3 py-1.5 text-[10px]"
          ...
          <table className="table table-hover align-middle mb-0">
            <thead className="bg-light">
              <tr className="text-[9px]">
                <th className="ps-3 border-0 py-2.5 fw-bold text-muted text-uppercase">
                  Semester
                </th>
                <th className="border-0 py-2.5 fw-bold text-muted text-uppercase d-none d-md-table-cell">
                  Tahun Ajaran
                </th>
                <th className="border-0 py-2.5 fw-bold text-muted text-uppercase d-none d-md-table-cell">
                  Status
                </th>
                <th className="border-0 py-2.5 text-end pe-3 fw-bold text-muted text-uppercase">
                  Aksi
                </th>
              </tr>
            </thead>
              ...
              {loading ? (
                <tr>
                  <td colSpan={4} className="text-center py-5">
                    <Loader2
                      className="text-warning animate-spin mb-2 mx-auto"
                      size={20}
                    />
                ...
                <tr>
                  <td
                    colSpan={4}
                    className="text-center py-5 text-muted text-[10px]"
                  >
                    Tidak ada data.
                  </td>
                </tr>
            ...
            <div className="modal-content border-0 shadow-lg rounded-3 overflow-hidden">
              <div className="modal-header border-0 pb-0 px-3 pt-3">
                ...
                <button
                  ...
                  className="btn-close shadow-none scale-75"
                  title="Tutup"
                ></button>
              </div>
              <div className="modal-body p-3 pt-2">
                ...
                <div className="mb-2 d-flex align-items-center mt-3">
                  <div className="form-check form-switch p-0 m-0 d-flex align-items-center">
                    <input
                      className="form-check-input ms-0 me-2 shadow-none cursor-pointer"
                      type="checkbox"
                      id={activeId}
                    ...
                    />
                    <label
                      className="form-check-label text-dark fw-semibold text-[10px] cursor-pointer"
                      htmlFor={activeId}
                    >
                      Aktifkan Semester
                    </label>
              ...
              <div className="modal-footer border-0 p-3 pt-0">
                <button
                  onClick={handleSave}
                  className="btn btn-warning btn-sm w-100 fw-bold shadow-sm py-2 text-[11px] rounded-3"
