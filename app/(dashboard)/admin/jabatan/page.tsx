...
import { 
  Plus, Edit2, Loader2, Trash2, Image as ImageIcon, Briefcase, Users, Crop, ChevronRight, Search
} from 'lucide-react';
  ...
  <tr>
    ...
    <td className="py-2 text-end pe-3">
      <div className="d-flex justify-content-end gap-1">
        <button onClick={() => onEdit(data)} className="btn btn-sm p-1 text-primary border-0 shadow-none" title="Edit Jabatan">
...
const StrukturRow = memo(({ data, onEdit, onDelete }: { data: any, onEdit: (d: any) => void, onDelete: (id: string) => void }) => (
  <tr className="align-top">
    <td className="ps-2 ps-md-3 py-2">
      <div className="d-flex align-items-start">
        ...
        <div className="ms-2">
          ...
          <div className="text-primary fw-medium text-[8px] mb-0.5">
    ...
    <td className="py-2 text-end pe-2 pe-md-3 align-middle">
      <div className="d-flex justify-content-end gap-1">
        <button onClick={() => onEdit(data)} className="btn btn-sm p-1 text-primary border-0 shadow-none" title="Edit Struktur">
...
export default function ManajemenOrganisasi() {
  ...
  const handleApplyCrop = async () => {
    if (tempImage && croppedAreaPixels) {
      try {
        ...
        const file = new File([croppedBlob], "ttd_pejabat.jpg", { type: "image/jpeg" });
  ...
  const handleEditClick = useCallback(async (item: any) => {
    ...
    if (activeTab === 'jabatan') {
      ...
      if (item.url_ttd) {
        try { 
          const base64 = await getBase64FromUrl(item.url_ttd); 
    ...
    <div className="container-fluid py-2 py-md-3 px-2">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <div className="d-flex align-items-center gap-2">
          <div className={`d-flex align-items-center cursor-pointer ${activeTab === 'jabatan' ? 'text-primary' : 'text-muted'}`} onClick={() => setActiveTab('jabatan')}>
          ...
          <div className={`d-flex align-items-center cursor-pointer ${activeTab === 'struktur' ? 'text-primary' : 'text-muted'}`} onClick={() => setActiveTab('struktur')}>
        ...
        <div>
          <button onClick={() => { 
            ...
            setShowForm(true); 
          }} className="btn btn-primary btn-sm px-2 shadow-sm rounded-3 py-1.5 text-[9px]">
              ...
              {loading ? (
                <tr>
                  <td colSpan={4} className="text-center py-5">
                    <Loader2 className="text-primary animate-spin mb-2 mx-auto" size={14} />
            ...
            <div className="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
              <div className="modal-header border-0 pb-0 px-3 pt-3">
                <h6 className="modal-title fw-bold text-dark text-[10px]">
                  {tempImage ? "Potong Tanda Tangan" : (isEdit ? "Edit Data" : "Tambah Data")}
                </h6>
              ...
              <div className="modal-body p-3 pt-2">
                {tempImage ? (
                  <div className="ui-cropper-wrapper">
                    <div className="position-relative w-100 bg-dark rounded-3 overflow-hidden h-[180px]">
                        <Cropper 
                            image={tempImage} 
                            crop={crop} 
                            zoom={zoom} 
                    ...
                    <div className="mt-3 d-flex flex-column gap-2">
                      <button onClick={handleApplyCrop} className="btn btn-primary btn-sm w-100 fw-bold text-[10px] py-2 rounded-3 shadow">
                        ...
                        <div>
                          ...
                          {photoPreview && (
                            <div className="ui-preview-foto mt-2" onClick={() => originalImage && setTempImage(originalImage)}>
                              ...
                              <button type="button" className="btn-close ui-btn-delete-preview shadow-none scale-75" aria-label="Hapus Gambar" onClick={(e) => { e.stopPropagation(); setFormData({...formData, file_ttd: null}); setPhotoPreview(null); setOriginalImage(null); }}></button>
                            </div>
                          )}
                        </div>
                      </>
              ...
              {!tempImage && (
                <div className="modal-footer border-0 p-3 pt-0">
                  <button onClick={handleSave} className="btn btn-primary btn-sm w-100 fw-bold shadow-sm py-2 text-[10px] rounded-3" disabled={isSubmitting}>
