...
import { 
  ...
  MessageCircle, FileText, Users, GraduationCap, School
} from 'lucide-react';

interface OrangtuaViewProps {
  user?: any;
}

const styles = {
  wrapper: "parent-dashboard-wrapper p-2",
...
export default function OrangtuaView() {
  ...
  const fetchData = useCallback(async (isMounted: boolean) => {
    try {
      const res = await api.ortu.getDashboard();
      if (isMounted) {
  ...
  const anakList = useMemo(() => dash?.anak_statistics || [], [dash]);
  const currentAnak = useMemo(() => anakList[selectedAnakIdx] || null, [anakList, selectedAnakIdx]);
  const stats = useMemo(() => currentAnak?.statistics || {}, [currentAnak]);
  ...
  return (
    <div className={styles.wrapper}>
        ...
        <div className="row g-2 mb-3">
          <div className="col-12">
            <div className="d-flex gap-2 overflow-auto pb-2 no-scrollbar">
              {anakList.map((anak: any, idx: number) => (
                <button
                  key={idx}
                  onClick={() => setSelectedAnakIdx(idx)}
                  className={`btn rounded-pill px-4 py-2 border-0 fw-bold transition-all flex-shrink-0 d-flex align-items-center gap-2 ${
                    selectedAnakIdx === idx ? 'bg-warning text-white shadow-sm' : 'bg-white text-muted shadow-sm'
                  }`}
                >
                  <Users size={16} />
                  {anak.nama_anak}
      ...
      <div className="row g-3 mb-3">
        ...
        <StatCard label="Poin Positif" value={stats.poin?.total_positif} icon={<BookOpen size={20} />} variant="stat-purple" trend="Prestasi" />
      </div>
                ...
                <div key={i} className="col-md-6">
                  <div className="p-2 rounded-2xl border border-light bg-light bg-opacity-25 d-flex align-items-center justify-content-between hover-lift">
                    <div className="d-flex align-items-center gap-3">
                      <div className="avatar-32 rounded-circle bg-warning text-white d-flex align-items-center justify-content-center">
                        <Calendar size={14} />
                      </div>
                      <div>
        ...
        <div className="col-lg-4">
          <div className={`${styles.card} p-3 position-relative overflow-hidden`}>
              ...
              <div className="position-relative z-1 d-flex flex-column h-100">
                <p className="fw-black text-muted x-small-text text-uppercase tracking-widest mb-3">Informasi Akademik Anak</p>
                <div className="mb-3">
                    <div className="fw-black text-dark h5 text-uppercase tracking-widest mb-0 leading-tight">
                        {currentAnak.nama_anak}
                    </div>
                ...
                <div className="vstack gap-2 mb-4">
                  ...
                  <LightMetric label="Wali Kelas" value={currentAnak.wali_kelas} />
